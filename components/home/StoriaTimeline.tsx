...
export function StoriaTimeline({ momenti }: { momenti: MomentoStoria[] }) {
  return (
    <ol className="relative ms-1">
      <span
        aria-hidden
        className="absolute top-2 bottom-2 start-[7px] w-0.5 bg-gradient-to-b from-orange via-teal to-teal-300"
      />
      {momenti.map((m, i) => (
        <li key={i} className="relative ps-8 pb-8 last:pb-0">
          <span
            aria-hidden
            className="absolute top-1 start-0 h-3.5 w-3.5 rounded-full border-2 border-orange bg-cream"
          />
